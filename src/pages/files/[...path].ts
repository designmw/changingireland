...
export const GET: APIRoute = async ({ params, request }) => {
  ...
  // Responsive images request one of the finite widths generated by
  // scripts/responsive-media.mjs (and by the admin uploader for new media).
  // A missing variant simply falls through to the existing full-size object,
  // so old uploads remain valid throughout the rollout.
  const requestedWidth = Number(new URL(request.url).searchParams.get('w'));
  const responsiveWidths = new Set([160, 320, 480, 720, 960, 1280]);
  const isRaster = /\.(png|jpe?g|webp)$/i.test(key);
  let object =
    isRaster && responsiveWidths.has(requestedWidth) ? await bucket.get(`${key}.w${requestedWidth}.webp`) : null;
  ...
  if (!object && wantsWebp) object = await bucket.get(`${key}.webp`);
  ...
  // Post and ad filenames are immutable (admin uploads are timestamped), so
  // browsers can retain them for a year. Magazine cover keys can be replaced
  // in-place when an issue is edited and deliberately keep the shorter policy.
  headers.set(
    'cache-control',
    key.startsWith('uploads/') || key.startsWith('ads/')
